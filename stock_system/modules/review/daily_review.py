...
def _fetch_stock_picks(pro, today: str, name_map: Dict) -> List[Dict]:
    ...
    try:
        ...
        # ── Step 1.5: 批量获取今日资金净流入（Tushare net_mf_amount，单位万元，一次调用）
        mf_map = {}
        try:
            mf_df = pro.moneyflow(trade_date=today, fields="ts_code,net_mf_amount")
        ...
        for ts_code in candidates:
            try:
                ...
                # 6. 资金维度 (20分，Tushare net_mf_amount，单位万元)
                mf_net = mf_map.get(ts_code)
                if mf_net is not None:
                    if mf_net > 50000:
                        score += 20; signals.append(f"资金净流入{mf_net/10000:.1f}亿")
                    elif mf_net > 10000:
                        score += 15; signals.append(f"资金净流入{mf_net/10000:.1f}亿")
                    elif mf_net > 2000:
                        score +=  8; signals.append(f"资金净流入{mf_net:.0f}万")
