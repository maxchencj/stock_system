...
# 今天11:30
time1 = datetime.combine(today, time(11, 30))
# 明天07:00
time2 = datetime.combine(tomorrow, time(7, 0))

# 如果今天11:30已经过了，就不设置
if time1 > datetime.now():
    scheduler.add_job(make_push("今天11:30"), DateTrigger(run_date=time1))
    logger.info(f"已设置定时推送: {time1.strftime('%Y-%m-%d %H:%M')} (今天11:30)")
else:
    logger.info("今天11:30已过，跳过")
